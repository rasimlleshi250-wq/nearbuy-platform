    ...
    <main className="nb-home">
      ...
      <section className="nb-hero">
        ...
        <div className="nb-hero-content">
          <div className="nb-hero-badge">🇦🇱 Platforma #1 në Shqipëri</div>
        ...
        <div className="nb-container">
          ...
          {loadingBiz ? (
            ...
            <div className="nb-empty-state">
              <p>🏪 Bizneset e para do të shfaqen së shpejti!</p>
            </div>
          ) : (
      ...
      <section className="nb-section nb-section-dark">
        <div className="nb-container">
          ...
          {loadingPro ? (
            ...
            <div className="nb-empty-state">
              <p>👷 Profesionistët e parë do të shfaqen së shpejti!</p>
            </div>
          ) : (
            <div className="nb-pro-grid">
              {professionals.map(p => (
                <div key={p.id} className="nb-pro-card">
                  ...
                  <div className="nb-pro-photo">
                    {p.photo ? <img src={p.photo} alt={p.name} /> : <span>👤</span>}
                  </div>
                  <div className="nb-pro-info">
                  ...
                  <div className="nb-pro-footer">
                    <span className="nb-verified nb-verified-pro">✓ Verifikuar</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        /* Categories */
        .nb-cat-grid{display:grid;grid-template-columns:repeat(8,1fr);gap:10px}
        .nb-cat-card{display:flex;flex-direction:column;align-items:center;gap:8px;padding:1.25rem 0.5rem;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;cursor:pointer;font-family:inherit;transition:all .2s}
        .nb-cat-card:hover{background:rgba(245,200,66,0.08);border-color:rgba(245,200,66,0.25);transform:translateY(-2px)}
        .nb-cat-icon{font-size:1.75rem}
        .nb-cat-name{font-size:0.75rem;font-weight:600;color:#a1a1aa}
        ...
        .nb-pro-photo{width:52px;height:52px;border-radius:50%;background:rgba(192,132,252,0.1);border:1px solid rgba(192,132,252,0.2);display:flex;align-items:center;justify-content:center;font-size:1.5rem;overflow:hidden}
        ...
        @media(max-width:900px){.nb-cat-grid{grid-template-columns:repeat(4,1fr)}.nb-biz-grid,.nb-pro-grid,.nb-how-grid,.nb-loading-row{grid-template-columns:repeat(2,1fr)}.nb-how-arrow{display:none}}
        @media(max-width:600px){.nb-nav{padding:0 1rem}.nb-nav-link{display:none}.nb-hero{padding:5rem 1rem 3rem}.nb-cat-grid{grid-template-columns:repeat(4,1fr)}.nb-biz-grid,.nb-pro-grid,.nb-how-grid,.nb-loading-row{grid-template-columns:1fr}.nb-cta-title{font-size:1.5rem}}
      `}</style>
    </main>
  );
}
