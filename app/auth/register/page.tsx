...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (code === "auth/invalid-email") setError("Adresa email nuk është e vlefshme.");
    ...
    <main className="nb-root">
      ...
      <div className="nb-card">
        ...
        <div className="nb-header">
          ...
          <p>Filloje falas — nuk kërkohet kartë krediti</p>
        </div>
        ...
        <button onClick={handleGoogle} disabled={googleLoading || loading} className="nb-google-btn" type="button">
          ...
          <span>{googleLoading ? "Duke u lidhur…" : "Regjistrohu me Google"}</span>
        </button>
        ...
        <form onSubmit={handleRegister} className="nb-form">
          <div className="nb-field">
            ...
            <div className="nb-input-wrap">
              ...
              <input id="name" type="text" autoComplete="name" placeholder="Emri Mbiemri" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
          </div>

          <div className="nb-field">
            <label htmlFor="email">Email <span className="nb-req">*</span></label>
            <div className="nb-input-wrap">
              ...
              <input id="email" type="email" autoComplete="email" required placeholder="emri@shembull.al" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
          </div>

          <div className="nb-field">
            <label htmlFor="password">Fjalëkalimi <span className="nb-req">*</span></label>
            <div className="nb-input-wrap">
              ...
              <input id="password" type={showPassword ? "text" : "password"} autoComplete="new-password" required placeholder="Min. 8 karaktere" value={password} onChange={(e) => setPassword(e.target.value)} />
          ...
          <div className="nb-field">
            <label htmlFor="confirm">Konfirmo fjalëkalimin <span className="nb-req">*</span></label>
            <div className="nb-input-wrap">
              ...
              <input id="confirm" type={showPassword ? "text" : "password"} autoComplete="new-password" required placeholder="Përsërit fjalëkalimin" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
          ...
          <button type="submit" disabled={loading || googleLoading} className="nb-btn-primary">
            ...
            {loading ? "Duke krijuar llogarinë…" : "Krijo llogarinë falas"}
          </button>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nb-card { position: relative; z-index: 1; background: rgba(20,20,20,0.97); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; padding: 2.5rem 2.25rem; width: 100%; max-width: 440px; box-shadow: 0 0 0 1px rgba(249,115,22,0.04), 0 25px 60px rgba(0,0,0,0.6); animation: up 0.45s cubic-bezier(.22,.68,0,1.15) both; }
        ...
        .nb-input-wrap input { width: 100%; padding: 0.7rem 0.9rem 0.7rem 2.4rem; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); border-radius: 11px; color: #f4f4f5; font-size: 0.875rem; outline: none; transition: border-color 0.2s, box-shadow 0.2s, background 0.2s; font-family: inherit; }
