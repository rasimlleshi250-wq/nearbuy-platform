...
const PROFESSIONS = ["Të gjitha", "Elektriçist", "Hidraulik", "Ndërtues", "Bojaxhi", "Rrobaqepës", "Mjek", "Avokat", "Arkitekt", "Inxhinier", "Mësues", "Kontabilist", "Tjetër"];
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const pros = snap.docs.map(d => ({ id: d.id, ...d.data() } as Professional));
        setProfessionals(pros);
        pros.forEach(p => trackView("professionals", p.id));
      } catch (e) {
        try {
          const q2 = query(collection(db, "professionals"), where("verified", "==", true));
          const snap2 = await getDocs(q2);
          const pros2 = snap2.docs.map(d => ({ id: d.id, ...d.data() } as Professional));
          setProfessionals(pros2);
          pros2.forEach(p => trackView("professionals", p.id));
    ...
    <div className="pro-root">
      ...
      <div className="pro-container">
        ...
        <div className="pro-filters">
          <div className="pro-search-wrap">
            ...
            <input type="text" placeholder="Kërko profesionist..." value={search}
            ...
            {filtered.map(p => (
              <div key={p.id} className={`pro-card ${p.featured ? "pro-card-featured" : ""}`}>
                ...
                <div className="pro-card-bottom">
                  ...
                  {p.experience && <span className="pro-exp">🏅 {p.experience} vjet</span>}
                </div>
                {p.services && p.services.length > 0 && (
                  <div className="pro-services">
                    {p.services.slice(0, 3).map(s => <span key={s} className="pro-service-tag">{s}</span>)}
                    {p.services.length > 3 && <span className="pro-service-more">+{p.services.length - 3}</span>}
                  </div>
                )}
                {p.phone && (
                  <a href={`tel:${p.phone}`} className="pro-contact-btn"
                    onClick={() => trackContact("professionals", p.id)}>
                    📞 Kontakto
                  </a>
                )}
              </div>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .pro-nav-link{font-size:0.85rem;color:#a1a1aa;text-decoration:none;padding:0.4rem 0.8rem;transition:color .2s}
        .pro-nav-link:hover{color:#fff}
        .pro-nav-login{font-size:0.85rem;color:#a1a1aa;text-decoration:none;padding:0.4rem 0.8rem}
        ...
        .pro-contact-btn{display:block;text-align:center;padding:0.6rem;background:rgba(192,132,252,0.1);border:1px solid rgba(192,132,252,0.25);border-radius:10px;color:#c084fc;font-size:0.85rem;font-weight:600;text-decoration:none;transition:background .2s;margin-top:auto}
